...
# explicit fourier->real map (slow Fourier transform).
# Can be implemented more efficiently with an FFT
function build_four_to_real()
    Q = zeros(ComplexF64, Nb, Nb)
    for ix = 1:Nb
        for iq = 1:Nb
...
A = build_ham()  # in q space
Q = build_four_to_real()
@assert Diagonal(diag(Q'Q)) ≈ (Q'Q)[1,1]*I #ensure unitarity up to constant factor
F2R = Q
R2F = inv(Q)
...
# Build density matrices and density
...
figure()
for iplot=1:5
    plot(xrange, real(F2R*V[:,iplot]), label="Eigenvector $iplot")
end
legend()
title("Bloch waves")
